...
from ollama_client import get_ollama_summary
    ...
    ui.tags.div(
        ...
        ui.tags.div(
            ui.tags.div(
                ...
                ui.tags.p(
                    "Type a word, pick one from the list, or use Random word to see pronunciations and definitions, then generate an AI summary.",
...
def server(input, output, session):
    ...
    ai_summary = reactive.value(None)
    ...
    def data_card():
        ...
        if prons or defs:
            children.append(
                ui.tags.div(
                    ui.input_action_button("ai_btn", "Generate AI summary", class_="btn btn-success mt-2"),
                    class_="mt-2",
                )
            )
    ...
    def _ai():
        ...
        ai_loading.set(True)
        try:
            summary = get_ollama_summary(res)
            ai_summary.set(summary)
        finally:
    ...
    def ai_card():
        if ai_loading.get():
            return ui.tags.div(
                ui.tags.div(
                    ui.tags.span("Generating AI summary...", class_="text-primary fw-medium"),
                    ui.tags.div(
                        ui.tags.div(class_="spinner-border spinner-border-sm text-primary", role="status"),
                        class_="mt-2",
                    ),
        ...
        if not text:
            return None
        return ui.tags.div(
            ui.tags.h5("AI report", class_="mb-2"),
            ui.tags.div(text, class_="ai-report"),
            class_="content-card",
        )
